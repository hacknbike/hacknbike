...
karte.setView([47.80, 13.41], 9);

 const wege = 'https://gis.tirol.gv.at/ogd/sport_freizeit\MTB_RT_MGIAustriaGKWest.json'; 
...
// URL : https://www.salzburg.gv.at/ogd/e5bc00bf-a84a-46fa-9494-b43ba606f1f6/Mountainbike_Strecken.json
//URL Daten Salzburg MTB Strecken

//const trails = 'https://www.salzburg.gv.at/ogd/e5bc00bf-a84a-46fa-9494-b43ba606f1f6/Mountainbike_Strecken.json';
/*
function linienPopup(feature,layer) {
  ...
  async function loadTrails(TRAILS) {
      const antwort = await fetch(TRAILS);
      const trailsData = await antwort.json();
      const trailsJson = L.geoJson(trailsData, {
          style: function() {
              return {
              color:"blue"
          };
      ...
      karte.addLayer(trailsJson);
      layerControl.addOverlay(trailsJson, "MTB_Trails");
...
for (let i=0; i < TRAILS.length; i++) {
    ...
    pulldown.innerHTML += `<option value="${i}">${TRAILS[i].OBJECTID}</option>`
}
...
//
function trailsErzeugen(Orgcode){
    let daten = TRAILS[Orgcode];
